file D:\repos\npm-packages\packages\md-exporter\demo\server\package.json:

```json
{
  "name": "md-exporter-demo-server",
  "version": "0.1.0",
  "type": "module",
  "scripts": {
...
file D:\repos\npm-packages\packages\md-exporter\demo\server\tsconfig.json:
...
file D:\repos\npm-packages\packages\md-exporter\demo\server\src\server.ts:
...
app.post("/api/run", async (req, res) => {
  try {
    ...
    if (options.filter === "glob" && !options.pattern) {
      return res
        .status(400)
        .json({ error: "pattern is required when filter=glob" });
    }
